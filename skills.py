...
def all_odd(number_list):
    if not number_list:
        return []
    if numbers_list[0] % 2 == 1:
        return [number_list[0]] + find_odds(numbers[1:])
    return all_odd(number_list[1:])
...
def all_even(number_list):
    ...
    if number_list[0] % 2 == 2:
        return [number_list[0]] + find_evens(number_list[2:])
    return all_even(number_list[2:])
...
def print_indeces(my_list):
    my_list = ["english" , "spanish" , "spanish" , "tagoloan"]
for i in range(len(my_list)):
    print print_indeces("my_list #%s is %" % (i, my_list[i]))
